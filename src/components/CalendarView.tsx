...
function mapTasksToAgenda(tasks: Task[]) {
  ...
  tasks.forEach((t) => {
    const dateKey = t.scheduledAt ? new Date(t.scheduledAt).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10);
...
export default function CalendarView({ mode, tasks, onTaskPress }: Props) {
  ...
  if (mode === 'agenda') {
    return (
      <Agenda
        items={agendaItems}
        renderItem={(item: any) => (
          <View style={{ padding: 12, backgroundColor: '#fff', borderRadius: 6, marginBottom: 8 }}>
            <View>
  ...
  return <View style={{ flex: 1 }}>{/* timeline fallback implemented by parent screen */}</View>;
}
