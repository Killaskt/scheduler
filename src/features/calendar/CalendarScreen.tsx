...
export const CalendarScreen = () => {
  ...
  // compute 14-day window starting at the Sunday of the selectedDate's week
  const days = useMemo(() => {
  ...
  const tasksByDay = useMemo(() => {
    ...
    tasks.forEach((t) => {
      ...
      const d = typeof t.scheduledAt === 'string' ? new Date(t.scheduledAt) : (t.scheduledAt as Date);
  ...
  const renderDayColumn = (day: Date) => {
    ...
    return (
      <View style={styles.dayColumn} key={key}>
        <TouchableOpacity onPress={() => setSelectedDate(day)} style={styles.dayHeader}>
            ...
            dayTasks.map((task) => (
              <View key={task.id} style={styles.calendarTask}>
                <Text style={styles.taskTime}>
                  {task.scheduledAt ? format(typeof task.scheduledAt === 'string' ? parseISO(task.scheduledAt) : (task.scheduledAt as Date), 'p') : ''}
                </Text>
  ...
  if (mode === 'agenda') {
    ...
    const agendaData = days.map((d) => ({ date: d, tasks: tasksByDay[d.toDateString()] || [] }));
    return (
      <View style={styles.container}>
        <View style={styles.topBar}>
          <Text style={styles.header}>Agenda</Text>
          <TouchableOpacity onPress={() => setMode('calendar')} style={styles.modeButton}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.agendaDay}>
              <Text style={styles.agendaDayHeader}>{format(item.date, 'EEEE, MMM d')}</Text>
              {item.tasks.length > 0 ? (
                item.tasks.map((task) => (
                  <View key={task.id} style={styles.agendaTask}>
                    <Text style={styles.taskTime}>{task.scheduledAt ? format(typeof task.scheduledAt === 'string' ? parseISO(task.scheduledAt) : (task.scheduledAt as Date), 'p') : ''}</Text>
                    <Text style={styles.taskTitle}>{task.title}</Text>
                  </View>
                ))
              ) : (
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topBar}>
        <Text style={styles.header}>2‑Week Calendar</Text>
        <TouchableOpacity onPress={() => setMode('agenda')} style={styles.modeButton}>
